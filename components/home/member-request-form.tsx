...
export function MemberRequestForm() {
  const locale = normalizeRequestLocale(useLocale())
  const copy = getMemberRequestCopy(locale)
  const skillOptions = getSkillOptions(locale)
  const defaultSkill = skillOptions[0]?.value ?? "developer-anchor"
  const formRef = React.useRef<HTMLFormElement>(null)
  const [selectedSkill, setSelectedSkill] = React.useState<SkillOptionValue>(defaultSkill)
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const payload: MemberRequestInput = {
      ...
      primarySkill: String(formData.get("primarySkill") || selectedSkill) as SkillOptionValue,
    ...
    try {
      const response = await fetch(`/api/member-requests?locale=${locale}`, {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        ...
        setFeedbackMessage(result?.message || copy.form.fallbackError)
      ...
      setFeedbackMessage(result?.message || copy.form.fallbackSuccess)
    } catch {
      setSubmissionState("error")
      setFeedbackMessage(copy.form.connectionError)
    }
  }

  function handleSkillChange(value: string | null) {
    if (typeof value === "string" && isSkillOptionValue(value)) {
      ...
      <div className="grid gap-6 sm:grid-cols-2">
        <div className="grid gap-2">
          <label htmlFor="twitter-handle" className="text-sm font-medium text-white/80">
            {copy.form.xHandleLabel}
          </label>
          <Input
            id="twitter-handle"
            name="xHandle"
            autoComplete="username"
            placeholder={copy.form.xHandlePlaceholder}
            required
            className={fieldClassName}
        ...
        <div className="grid gap-2">
          ...
          >
            ...
            >
              <SelectValue placeholder={copy.form.primarySkillPlaceholder} />
            </SelectTrigger>
